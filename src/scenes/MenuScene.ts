...
export class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    menuEntry(this, 69, 302, '01', journey.deliveries.desert ? '带着回信，再去远方' : journey.deliveries.mountain ? '下一封，寄往晒被沙原' : journey.deliveries.lake ? '下一封，寄往云阶山' : journey.deliveries.forest || postal.deliveryCompleted ? '下一封，寄往圆镜湖' : '今天，寄往风铃森林', '打开邮路图 · 读懂地址，选一条路，和小暖亲手送达', () => this.scene.start('JourneyMapScene'), true);
    ...
    const deliveredLetters = ['forest', 'lake', 'mountain', 'desert'].filter(id => journey.deliveries[id as keyof typeof journey.deliveries]).length;
    label(this, 33, 740, `亲手送达 ${deliveredLetters}/4 封信    ·    练习星章 ${campaign.totalStars}    ·    暖晶 ${state.shadowCores}`, 12, UI.ink);
