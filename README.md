...
当前候选版 **1.3.0-rc.2**：五章五十个独立小关卡、战役 / 无尽 / 影子切磋三种模式，以及装备、营地研究和角色专精。本轮新增本机旅途备份与恢复，修复设置键盘焦点、敌人队列、无尽满卡转场与高站点边界、切磋纪录保存提示和战斗通知重叠，并缓存静态花园以减少每帧绘制成本。**108 项测试、TypeScript / 生产构建、静态检查和依赖审计通过，0 已知依赖漏洞**；最终静态包与标准 ZIP 已生成。准确证据与待测项见 [发行准备](docs/release-readiness.md)。
...
最后修复了圆形碰撞把营地挤离中心的问题：同条件定向复现中，营地保持在地图中心。此前二十分钟和生产原生流程报告早于这项修复；最后整包图形复测受工具自动审批拦截，仍未执行。完整证据及发布门槛见发行准备。
